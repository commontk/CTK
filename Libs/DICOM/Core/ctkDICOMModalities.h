/*=========================================================================

  Library:   CTK

  Copyright (c) Kitware Inc.

  Licensed under the Apache License, Version 2.0 (the "License");
  you may not use this file except in compliance with the License.
  You may obtain a copy of the License at

      http://www.apache.org/licenses/LICENSE-2.0.txt

  Unless required by applicable law or agreed to in writing, software
  distributed under the License is distributed on an "AS IS" BASIS,
  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
  See the License for the specific language governing permissions and
  limitations under the License.

=========================================================================*/

#ifndef __ctkDICOMModalities_h
#define __ctkDICOMModalities_h

// Qt includes
#include <QStringList>

// CTK includes
#include "ctkDICOMCoreExport.h"

/// \ingroup DICOM_Core
///
/// \brief DICOM modality code definitions
///
/// This header provides comprehensive lists of DICOM modality codes
/// as defined in DICOM PS3.16 Annex D - DICOM Controlled Terminology Definitions.
/// Reference: https://dicom.nema.org/medical/dicom/current/output/chtml/part16/chapter_D.html
///
namespace ctkDICOMModalities
{
  /// Complete list of all current (non-retired) DICOM modality codes.
  CTK_DICOM_CORE_EXPORT const QStringList& allModalities();

  /// Modalities that should be excluded from thumbnail generation
  /// because they do not have meaningful image thumbnails.
  CTK_DICOM_CORE_EXPORT const QStringList& excludedFromThumbnailGeneration();

  /// Common imaging modalities typically used for filtering in UI.
  CTK_DICOM_CORE_EXPORT const QStringList& commonImagingModalities();

#if CTK_DEPRECATED_SINCE(0, 1)
  /// \deprecated Use ctkDICOMModalities::allModalities() instead.
  /// This variable will be removed in a future CTK release.
  /// Suppress this deprecation warning by building with
  /// -DCTK_DISABLE_DEPRECATED_BEFORE=0x000100
  Q_DECL_DEPRECATED_X("Use ctkDICOMModalities::allModalities() instead")
  CTK_DICOM_CORE_EXPORT extern const QStringList& AllModalities;

  /// \deprecated Use ctkDICOMModalities::excludedFromThumbnailGeneration() instead.
  Q_DECL_DEPRECATED_X("Use ctkDICOMModalities::excludedFromThumbnailGeneration() instead")
  CTK_DICOM_CORE_EXPORT extern const QStringList& ExcludedFromThumbnailGeneration;

  /// \deprecated Use ctkDICOMModalities::commonImagingModalities() instead.
  Q_DECL_DEPRECATED_X("Use ctkDICOMModalities::commonImagingModalities() instead")
  CTK_DICOM_CORE_EXPORT extern const QStringList& CommonImagingModalities;
#endif // CTK_DEPRECATED_SINCE(0, 1)
}

#endif // __ctkDICOMModalities_h
